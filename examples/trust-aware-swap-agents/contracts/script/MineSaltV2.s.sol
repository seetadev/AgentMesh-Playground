...
/// @notice Mine salt off-chain for AgentCounterV2 hook deployment
/// @dev V2 flags: afterInitialize + beforeSwap + afterSwap
contract MineSaltV2Script is Script {
    // Sepolia PoolManager
    ...
    function run() public pure {
        // Required flags for AgentCounterV2 (afterInitialize + beforeSwap + afterSwap)
        uint160 flags = uint160(Hooks.AFTER_INITIALIZE_FLAG | Hooks.BEFORE_SWAP_FLAG | Hooks.AFTER_SWAP_FLAG);

        bytes memory constructorArgs = abi.encode(POOL_MANAGER);
        bytes memory creationCode = abi.encodePacked(type(AgentCounterV2).creationCode, constructorArgs);
        bytes32 initCodeHash = keccak256(creationCode);

        console.log("Mining salt for AgentCounterV2 hook deployment...");
